...
class avatarUploadMiddleware {
  async uploadFile(req, res, next) {
    ...
    try {
      ...
      const fileType = checkImgFormat(base64Data);
      if (!fileType) {
        return res
          .status(400)
          .json("Ошибка загрузки файла. Обратитесь к администратору");
      }
      ...
      if (id) {
        ...
        if (currentUser.dataValues.avatar) {
          // относительный путь
          const pathToLocalFile = path.join(
            currentUser.dataValues.avatar.replace(url, "")
          );
      ...
      fs.writeFileSync(
        ...
        { recursive: true, encoding: "base64" }
      );
